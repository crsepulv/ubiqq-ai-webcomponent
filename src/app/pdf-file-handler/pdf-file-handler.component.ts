...
import { StorageLocationResponse } from '../file-transcriptor/types';

@Component({
  selector: 'app-pdf-file-handler',
  templateUrl: './pdf-file-handler.component.html',
  styleUrls: ['./pdf-file-handler.component.scss'],
})
export class PdfFileHandlerComponent {
  ...
  constructor(private http: HttpClient, public fb: FormBuilder) {
    ...
    this.metaDataForm = this.fb.group({
      nameOfFile: ['', [Validators.required, Validators.minLength(3)]],
    });
  }
  ...
  validateSelectors() {
    const nameOfFile = this.metaDataForm.get('nameOfFile')?.value;

    return nameOfFile && nameOfFile !== '' ? true : false;
  }
  ...
  uploadFile() {
    ...
    const encodedNameOfFile = encodeURIComponent(nameOfFile);

    if (this.validateSelectors()) {
      const metadata = {
        userid: this.userid,
        fileid: this.fileId,
        alias: encodedNameOfFile,
      };
  ...
  checkFileSize(file: File): boolean {
    const maxFileSizeInBytes = 100 * 1024 * 1024; // Convertir 300 MB a bytes
